...
- **Remediation** (`/management/remediation`, `Remediation.jsx`) — heads assign a drill-taker a focus (a game or Procedures) with a note/due date; the agent sees an **Assigned Practice** card on their Dashboard (`get_my_remediation()`). Completion is **auto + manual**: auto-completes once the agent finishes `target_sessions` qualifying completed drills (a session touching the target game; any completed session for a Procedures focus), and heads can mark complete/cancel. The assign form pre-suggests each agent's weakest game (reusing the same per-game accuracy computation as WeakAreas/Dashboard). Table `remediation_assignments` + RPCs `assign_remediation` / `set_remediation_status` / `list_remediation` / `get_my_remediation` (migration `supabase/add_remediation.sql`; the two internal helpers `remediation_progress` / `remediation_autocomplete` are locked to owner-only). Writes `REMEDIATION_ASSIGNED` / `REMEDIATION_COMPLETED` / `REMEDIATION_CANCELLED`.

Shared: `src/lib/exportXlsx.js` — reusable SheetJS export helper used by the new pages. All four migrations + the redeployed `admin-users` edge function were applied to production 2026-07-23.
