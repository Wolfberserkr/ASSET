...
- **Output:** `../ASSET-User-Manual.pdf` (55 pages)
- **Accuracy record:** `SOURCE-NOTES.md` — where each stated rule was verified, plus the
  app-behaviour items the manual documents that may be worth changing in code.
...
- Callouts: `.callout` (tip, blue), `.callout.note` (violet), `.callout.important` (amber),
  `.callout.danger` (red — irreversible actions).
...
```js
// render.mjs — node render.mjs docs/manual/index.html docs/ASSET-User-Manual.pdf
...
await page.pdf({
  ...
  footerTemplate: `<div style="width:100%;font-family:Arial,sans-serif;font-size:7pt;color:#8a93a3;
    ...
    <span>A.S.S.E.T. — User Manual &amp; Management Guide · July 2026</span>
...
```sh
chromium --headless --no-pdf-header-footer --print-to-pdf=docs/ASSET-User-Manual.pdf \
